...
describe('HybridLogicalClock', () => {
  ...
  test('send', () => {
    ...
    fakeNow.mockReturnValueOnce(1603265000);
    hlc = HLC.next(hlc);
    const ts = HLC.serialize(hlc);
    expect(ts).toBe('1fovof800112345678');
    expect(HLC.parse(ts)).toEqual({ counter: 1, id: '12345678', time: 1603265000 });
    fakeNow.mockReturnValueOnce(1603265000);
    hlc = HLC.next(hlc);
    expect(hlc).toEqual({ counter: 2, id: '12345678', time: 1603265000 });
